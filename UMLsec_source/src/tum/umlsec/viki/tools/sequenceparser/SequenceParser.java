/**
 * UML model parser for subsystems and sequence diagrams:
 *  - computes all possible flows of the subsystem and returns the ones that fit the sequence diagram
 *  - more UML diagrams (must only be statechart diagrams representing the subsystem and an additional sequence diagram representing the asynchronous messages checked for) can be stored in the MDR repository
...
package tum.umlsec.viki.tools.sequenceparser;
...
class Flow {
    ...
    // messages received in one step
    private Vector msgin;
    ...
    public boolean isEqual(Flow f) {
        ...
        for (int i = 0; i < msgin.size(); i++) {
            if (((Vector)msgin.elementAt(i)).size() != ((Vector)f.msgin.elementAt(i)).size()) return(false);
            for (int j = 0; j < ((Vector)msgin.elementAt(i)).size(); j++) {
                if (((String)((Vector)msgin.elementAt(i)).elementAt(j)).equals(((String)((Vector)f.msgin.elementAt(i)).elementAt(j))) == false) return(false);
            }
        }
    ...
    // method to add messages to the flow of messages
    public void addMsgIn(Vector v) {
        msgin.add(v);
    }
    
    // method to get messages at a certain step
    public Vector msginat(int i) {
        return((Vector)msgin.elementAt(i));
    }
    
    // method to get the size of the vector 'msgin'
    public int msginsize() {
        return(msgin.size());
    }
    
    // method to check if flow has reached the final state
    public boolean atFinalState() {
        if (isFinalState((StateVertex)flow.lastElement()) == true) return(true);
        else return(false);
    }
    ...
    // method to copy a vector of vectors
...
public class SequenceParser {
	...
	if(_foundIterations == false) {
        ...
        SequenceMessage[] messages = new SequenceMessage[n];
        for (int m = 0; m < n; m++) {
            messages[m] = new SequenceMessage();
        }
        for (Iterator it_sti_I = stimulusClasses.refAllOfClass().iterator(); it_sti_I.hasNext();) {
            Stimulus sti = (Stimulus)it_sti_I.next();
            int m = Integer.parseInt(sti.getName());
            messages[m-1].sender = sti.getSender().getName();
            messages[m-1].receiver = sti.getReceiver().getName();
            messages[m-1].messageName = sti.getDispatchAction().getName();
        }
        ...
        messages = null;
        printMatches(flows, msgs, _mainOutput);
        _mainOutput.writeLn();
        _mainOutput.writeLn();
        ...
        for (int i = 0; i < next.size(); i++) {
            ...
            for (int j = 0; j < oflow.size(); j++) {
                if (((Flow)oflow.elementAt(j)).getBuffer().equals("") == false) {
                    ...
                    String st2 = (String)diagrams.elementAt(j) + "." + getMsgOnly(st);
                    ((Vector)buffer.elementAt(getIndex(getMsgReceiver(st)))).add(st2);
            ...
            for (int j = 0; j < buffer.size(); j++) {
                if (((Vector)buffer.elementAt(j)).isEmpty()) {
                    ...
                    ftemp.addMsgIn(((Vector)buffer.elementAt(j)));
                    temp2.add(ftemp);
                    flowperm.add(temp2);
                } else if (((Vector)buffer.elementAt(j)).size() == 1) {
                    Vector temp2 = new Vector();
                    Flow ftemp = new Flow((Flow)oflow.elementAt(j));
                    ftemp.addInput(getMsgsOnly((Vector)buffer.elementAt(j)));
                    ftemp.addMsgIn(((Vector)buffer.elementAt(j)));
                    temp2.add(ftemp);
                    flowperm.add(temp2);
                } else {
    ...
    private Vector permut(Flow f, Vector buffer) {
        ...
        for (int i = 0; i < bufres.size(); i++) {
            ((Flow)res.elementAt(i)).addInput(getMsgsOnly((Vector)bufres.elementAt(i)));
            ((Flow)res.elementAt(i)).addMsgIn((Vector)bufres.elementAt(i));
        }
        return(res);
    }
    ...
    // method to get the messages without the receivers
    private static Vector getMsgsOnly(Vector v) {
        Vector res = new Vector();
        for (int i = 0; i < v.size(); i++) {
            String st = getMsgOnly((String)v.elementAt(i));
            res.add(st);
        }
        return(res);
    }
    ...
    // method to check if all statechart diagrams are at their final states
    private static boolean atFinalState(Vector oflow) {
        for (int i = 0; i < oflow.size(); i++) {
            if (((Flow)oflow.elementAt(i)).atFinalState() == false) return(false);
        }
        return(true);
    }
    
    // method to get the sent messages during one possible flow
    private Vector getMessages(Vector v) {
        Vector allmsgs = new Vector();
        for (int i = 0; i < ((Flow)v.firstElement()).msginsize(); i++) {
            Vector msgsstep = new Vector();
            for (int j = 0; j < diagrams.size(); j++) {
                if (((Flow)v.elementAt(j)).msginat(i).size() > 0) {
                    Vector msglist = new Vector();
                    for (int k = 0; k < ((Flow)v.elementAt(j)).msginat(i).size(); k++) {
                        SequenceMessage msg = new SequenceMessage();
                        msg.messageName = getMsgOnly((String)((Vector)((Flow)v.elementAt(j)).msginat(i)).elementAt(k));
                        msg.sender = getMsgReceiver((String)((Vector)((Flow)v.elementAt(j)).msginat(i)).elementAt(k));
                        msg.receiver = (String)diagrams.elementAt(j);
                        msglist.add(msg);
                    }
                    msgsstep.add(msglist);
                }
            }
            if (msgsstep.isEmpty() == false) allmsgs.add(msgsstep);
        }
        return(allmsgs);
    }
    
    // method to compare two flows of messages
    private static boolean chkMsgs(Vector msgs, Vector smsgs) {
        for (int i = 0; i < smsgs.size(); i++) {
            boolean equal = false;
            for (int j = 0; j < ((Vector)msgs.firstElement()).size(); j++) {
                if (((SequenceMessage)((Vector)((Vector)msgs.firstElement()).elementAt(j)).firstElement()).isEqual((SequenceMessage)smsgs.elementAt(i)) == true) {
                    equal = true;
                    ((Vector)((Vector)msgs.firstElement()).elementAt(j)).remove(0);
                    if (((Vector)((Vector)msgs.firstElement()).elementAt(j)).isEmpty() == true) {
                        ((Vector)msgs.firstElement()).remove(j);
                        if (((Vector)msgs.firstElement()).isEmpty() == true) {
                            msgs.remove(0);
                            if (msgs.isEmpty() == true) {
                                if (i == smsgs.size() - 1) return(true);
                                else return(false);
                            }
                        }
                    }
        ...
        if (msgs.isEmpty() == true) return(true);
        else return(false);
    }
    ...
    // method to print the possibilities of the subsystem which match the given sequence of messages
    private void printMatches(Vector v, Vector smsgs, ITextOutput _textOutput) {
        _textOutput.writeLn("The following possibilities of the subsystem fit the given sequence diagram.");
        _textOutput.writeLn();
        _textOutput.write("Sequence of messages: ");
        for (int i = 0; i < smsgs.size() - 1; i++) {
            ((SequenceMessage)smsgs.elementAt(i)).printMessage(_textOutput);
            _textOutput.write(", ");
        }
        if (smsgs.size() > 0) ((SequenceMessage)smsgs.elementAt(smsgs.size() - 1)).printMessage(_textOutput);
        _textOutput.writeLn();
        _textOutput.writeLn();
        int n = 1;
        for (int i = 0; i < v.size(); i++) {
            if (chkMsgs(getMessages((Vector)v.elementAt(i)), smsgs) == true) {
        ...
        if (n == 1) _textOutput.writeLn("No possibilities were found.");
    }
