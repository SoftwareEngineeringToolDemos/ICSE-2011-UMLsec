...
public class ExpressionParser {
    ...
    public boolean isEqual(ExpressionParser e) {
        ...
        while (e1.hasMoreElements()) {
            if (e2.hasMoreElements() == false) return(false);
            e1.nextElement();
            e2.nextElement();
        }
        if (e2.hasMoreElements() == true) return(false);
        e1 = vars.keys();
        while (e1.hasMoreElements()) {
            String st = (String)e1.nextElement();
            if (e.vars.get(st) == null) return(false);
            if (vars.get(st).equals(e.vars.get(st)) == false) return(false);
        }
        return(true);
    }
    ...
    // assignment: contains the names of the variables to be assigned, '=' and valid expressions separated by ';'
    public void assignment(String assignment) {
        ...
        while (assignbg.length() > 0) {
            ...
            if (assignbg.indexOf(";") != -1) {
                assignb = new StringBuffer(assignbg.substring(0, assignbg.indexOf(";")));
            ...
            // checking the syntax of the assigments
            if (assignb.indexOf("=") != -1) {
                ...
                StringBuffer expr = new StringBuffer(assignb.substring(assignb.indexOf("=") + 1));
                replaceVars(expr);
                reduceSigns(expr);
                changeSigns(expr);
                parenthesisa(expr);
    ...
    public boolean condition(String condition) {
        ...
        changeSigns(expr);
        parenthesisc(expr);
        if (expr.toString().equals("t")) return true;
        else return false;
    }
    ...
    private void replaceVars(StringBuffer expression) {
        // variable indicating if current position is in-between a name of a variable
        boolean atVar = false;
        // variable indicating if current position is last possible position
        boolean atEnd = false;
        // variable containing current position
        int n = 0;
        // variable containing position of first character of the current variable
        int vstart = 0;
        while (atEnd == false) {
            if (atVar == false) {
                if (n == expression.length()) atEnd = true;
                else {
                ...
                if (n == expression.length()) {
                    ...
                    if (tvar.equals("true")) expression.replace(vstart, n,  "t");
                    else if (tvar.equals("false")) expression.replace(vstart, n,  "f");
    ...
    private void reduceSigns(StringBuffer expression) {
        // variable containing current position
        int n = 0;
        // variable indicating if current position is in-between a series of '+' and '-'
        ...
        while (n < expression.length()) {
            if (atSign == false) {
                if ((expression.charAt(n) == '+') || (expression.charAt(n) == '-')) {
                    atSign = true;
                    fsign = n;
                    if (expression.charAt(n) == '-') sign = -1;
                    else sign = 1;
                }
                n++;
            } else {
                if ((expression.charAt(n) == '+') || (expression.charAt(n) == '-')) {
                    if (expression.charAt(n) == '-') sign = sign * (-1);
                } else {
                    String sig;
                    if ((n - fsign) > 1) {
                        if (sign < 0) sig = "-";
    ...
    private void changeSigns(StringBuffer expression) {
        ...
        int n = 0;
        while (n < expression.length()) {
            // replacing '+'/'-' following digits with 'p'/'m' (addition/subtraction)
            if (Character.isDigit(expression.charAt(n))) {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '+') {
            ...
            // replacing "<=" with "l" (less or equal)
                ...
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '=') expression.replace(n, n + 2, "l");
            }
            // replacing ">=" with "g" (greater or equal)
            else if (expression.charAt(n) == '>') {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '=') expression.replace(n, n + 2, "g");
            }
            // replacing "!=" with "u" (unequal)
            else if (expression.charAt(n) == '!') {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '=') expression.replace(n, n + 2, "u");
            }
            // replacing "==" with "e" (equal)
            else if (expression.charAt(n) == '=') {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '=') expression.replace(n, n + 2, "e");
            }
            // replacing "&&" with "a" (logical and)
            else if (expression.charAt(n) == '&') {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '&') expression.replace(n, n + 2, "a");
            }
            // replacing "||" with "o" (logical or)
            else if (expression.charAt(n) == '|') {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '|') expression.replace(n, n + 2, "o");
            }
            // replacing '+'/'-' following ')' with 'p'/'m' (addition/subtraction)
            else if (expression.charAt(n) == ')') {
                if (n < expression.length() - 1) if (expression.charAt(n + 1) == '+') {
                    expression.setCharAt(n + 1, 'p');
                    n++;
    ...
    private void eval1(StringBuffer expression) {
        for (int n = 0; n + 1 < expression.length(); n++) {
            if (expression.charAt(n) == '!') {
                if (expression.charAt(n + 1) == 't') expression.replace(n, n + 2, "f");
                else if (expression.charAt(n + 1) == 'f') expression.replace(n, n + 2, "t");
            }
        }
    }
        ...
        while (n < expression.length()) {
            if (Character.isDigit(expression.charAt(n)) == false) {
                if ((expression.charAt(n) == '*') || (expression.charAt(n) == '/') || (expression.charAt(n) == '%')) {
                    int farg = Integer.parseInt(expression.substring(fdigit, n));
                    int m = n + 1;
                    if ((expression.charAt(m) == '+') || (expression.charAt(m) == '-')) m++;
                    boolean edigit = false;
                    while (edigit == false) {
                        if (m == expression.length()) edigit = true;
                        else {
                            if (Character.isDigit(expression.charAt(m)) == false) edigit = true;
                            else m++;
                        }
                    }
                    int sarg = Integer.parseInt(expression.substring(n + 1, m));
                    int res = 0;
                    if (expression.charAt(n) == '*') res = farg * sarg;
                    else if (expression.charAt(n) == '/') res = farg / sarg;
                    else if (expression.charAt(n) == '%') res = farg % sarg;
                    if (res <= 0) {
                        if (fdigit > 0) {
                            if (expression.charAt(fdigit - 1) == '+') fdigit--;
    ...
    private void eval3(StringBuffer expression) {
        ...
        while (n < expression.length()) {
            if (Character.isDigit(expression.charAt(n)) == false) {
                if ((expression.charAt(n) == 'p') || (expression.charAt(n) == 'm')) {
                    if (fdigit > 0) if (expression.charAt(fdigit - 1) == '-') fdigit--;
                    int farg = Integer.parseInt(expression.substring(fdigit, n));
                    int m = n + 1;
                    if ((expression.charAt(m) == '+') || (expression.charAt(m) == '-')) m++;
                    boolean edigit = false;
                    ...
                    int sarg = Integer.parseInt(expression.substring(n + 1, m));
                    int res = 0;
                    if (expression.charAt(n) == 'p') res = farg + sarg;
                    else if (expression.charAt(n) == 'm') res = farg - sarg;
                    if ((fdigit > 0) && (farg > 0)) if (expression.charAt(fdigit - 1) == '+') fdigit--;
                    expression.replace(fdigit, m, Integer.toString(res));
                    n = fdigit + Integer.toString(res).length();
                    if (res < 0) fdigit++;
                }
    ...
    // method to evaluate '<', 'l', '>' and 'g' (less than, less or equal, greater than and greater or equal)
    // expression: contains a temporary expression
    private void eval4(StringBuffer expression) {
        ...
        while (n < expression.length()) {
            if (Character.isDigit(expression.charAt(n)) == false) {
                if ((expression.charAt(n) == '<') || (expression.charAt(n) == 'l') || (expression.charAt(n) == '>') || (expression.charAt(n) == 'g')) {
                    if (fdigit > 0) if (expression.charAt(fdigit - 1) == '-') fdigit--;
                    int farg = Integer.parseInt(expression.substring(fdigit, n));
                    int m = n + 1;
                    if ((expression.charAt(m) == '+') || (expression.charAt(m) == '-')) m++;
                    ...
                    boolean res = true;
                    if (expression.charAt(n) == '<') res=(farg < sarg);
                    else if (expression.charAt(n) == 'l') res=(farg <= sarg);
                    else if (expression.charAt(n) == '>') res=(farg > sarg);
                    else if (expression.charAt(n) == 'g') res=(farg >= sarg);
                    if ((fdigit > 0) && (farg > 0)) if (expression.charAt(fdigit - 1) == '+') fdigit--;
                    if (res == true) expression.replace(fdigit, m, "t");
                    else expression.replace(fdigit, m, "f");
                    n = fdigit + 1;
                }
                else {
                    fdigit = n + 1;
    ...
    private void eval5(StringBuffer expression) {
        ...
        while (n < expression.length()) {
            if (Character.isDigit(expression.charAt(n)) == false) {
                if ((expression.charAt(n) == 'e') || (expression.charAt(n) == 'u')) {
                    if (fdigit > 0) if (expression.charAt(fdigit - 1) == '-') fdigit--;
                    int farg = Integer.parseInt(expression.substring(fdigit, n));
                    int m = n + 1;
                    if ((expression.charAt(m) == '+') || (expression.charAt(m) == '-')) m++;
                    ...
                    int sarg = Integer.parseInt(expression.substring(n + 1, m));
                    boolean res = true;
                    if (expression.charAt(n) == 'e') res=(farg == sarg);
                    else if (expression.charAt(n) == 'u') res=(farg != sarg);
                    if ((fdigit > 0) && (farg > 0)) if (expression.charAt(fdigit - 1) == '+') fdigit--;
                    if (res == true) expression.replace(fdigit, m, "t");
                    else expression.replace(fdigit, m, "f");
                    n = fdigit + 1;
    ...
    private void eval6(StringBuffer expression) {
        for (int n = 1; n + 1 < expression.length(); n++) {
            if (expression.charAt(n) == 'a') {
                if ((expression.charAt(n - 1) == 't') && (expression.charAt(n + 1) == 't'))  {
                    expression.replace(n - 1, n + 2, "t");
                    n--;
                }
                else if ((expression.charAt(n - 1) == 't') && (expression.charAt(n + 1) == 'f'))  {
                    expression.replace(n - 1, n + 2, "f");
                    n--;
                }
                else if ((expression.charAt(n - 1) == 'f') && (expression.charAt(n + 1) == 't'))  {
                    expression.replace(n - 1, n + 2, "f");
                    n--;
                }
                else if ((expression.charAt(n - 1) == 'f') && (expression.charAt(n + 1) == 'f'))  {
                    expression.replace(n - 1, n + 2, "f");
                    n--;
                }
            }
    ...
    // method to evaluate parenthesis for assignments (evaluate the innermost expression until all parenthesis are resolved)
    // expression: contains a temporary expression
        ...
        while (op == true) {
            ...
            if (ip == -1) {
                ...
                while (n < expression.length()) {
                    ...
                    else if (expression.charAt(n) == ')') openp--;
                    n++;
                }
                int np = expression.indexOf(")", ip + 1);
                StringBuffer pexpr = new StringBuffer(expression.substring(ip + 1, np));
                eval2(pexpr);
                eval3(pexpr);
                if (pexpr.charAt(0) == '-') {
                    if (ip > 0) {
                        ...
                        else if (expression.charAt(ip - 1) == '+') ip--;
                        else if (expression.charAt(ip - 1) == 'p') {
                            pexpr.setCharAt(0, 'm');
                            ip--;
                        }
                    }
                } else if (pexpr.charAt(0) == '+') pexpr.deleteCharAt(0);
                expression.replace(ip, np + 1, pexpr.toString());
            }
        }
    }
    
    // method to evaluate parenthesis for conditions (evaluate the innermost expression until all parenthesis are resolved)
    // expression: contains a temporary expression
