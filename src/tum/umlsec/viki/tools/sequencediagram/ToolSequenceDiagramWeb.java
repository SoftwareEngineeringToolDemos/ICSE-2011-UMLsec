...
public class ToolSequenceDiagramWeb implements IVikiToolBase, IVikiToolConsole  {
...
//		_parameters.add(parameterFilename);
		...
		commandParser = CommandDescriptor.CommandDescriptorConsole(CID_SEQUENCE_PARSER, "Parse", "Parse Messages", true, parametersEmpty);
...
//			filename = outputTptp.checkFileExist(_parameters);
			String filename = System.getProperty("user.dir")+File.separator+"result.tptp";
			
//			if (filename.equalsIgnoreCase("exist!"))						
//				_mainOutput.writeLn("The file has exist! Pleas give a new name or output in other path.");
//			else
//			{
				...
				boolean is_ok=outputTptp.checkFile(_mainOutput);
				if (is_ok) {
					...
					_mainOutput.writeLn("Parsing failed!");
					_mainOutput.writeLn("The tool isn't able to copy the file.");
				}
